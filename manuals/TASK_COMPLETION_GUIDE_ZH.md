...
Task 1 和 Task 2 的代码现在已经在 `src/main.cpp` 中启用，可以直接参与编译和烧录。代码旁边仍保留了中文行内注释，方便你在烧录前快速确认每一行的作用。

已启用的任务代码位置如下：

| 任务 | 文件 | 已启用代码行号 | 说明 |
|---|---|---:|---|
| Task 1 启动旋律 | `src/main.cpp` | 30-44 | 先清空旧旋律队列，再在点击 START 后播放三段双电机和声旋律。 |
| Task 2 前进分支 | `src/main.cpp` | 69-71 | `speed > 0` 时让电机前进、点亮 LED、打印 FWD 日志。 |
| Task 2 后退分支 | `src/main.cpp` | 77-79 | `speed < 0` 时让电机后退、点亮 LED、打印 BWD 日志。 |
| Task 2 停止分支 | `src/main.cpp` | 85-87 | `speed == 0` 时停止电机、熄灭 LED、打印 STOP 日志。 |
| Bonus MIDI 歌曲 | `src/main.cpp` | 160-163 | 这仍然是注释状态；它是可选彩蛋，不属于 Task 1/2 必做内容。 |

烧录前你不需要再取消 Task 1/2 的注释；它们已经是正式运行代码。MIDI Bonus 仍然保持注释，因为当前还没有生成并启用 `src/song.h`。
